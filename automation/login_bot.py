"""
Bot de login SEAP-RJ — ASP.NET WebForms (multi-página).
...
6. Submeter 'Avançar'
7. Se solicitado, navegar para filtro (FrmEventoAssociar.aspx) + segundo CAPTCHA
...
class SeapLoginBot:
    """
    Bot de login do SEAP-RJ — ASP.NET WebForms (multi-página).

    Fluxo:
      Página 1: navegar → selecionar tipo → credenciais → CAPTCHA → submit
                com retry guiado pela resposta real do site
    ...
    # SECTION: Seletores WebForms reais (DOM dump 15/05/2026)
    ...
    async def execute_login(
        ...
        try:
            # ── PÁGINA 1: Login com retry guiado ──
            ...
            for attempt in range(1, self.MAX_CAPTCHA_RETRIES + 1):
                ...
                # Loop não fez break → todas as tentativas falharam
                elapsed = time.time() - start
                await self._save_failure_artifact("debug_after_submit.png")
                failure_message = (
                    last_evaluation.message
            ...
            return LoginResult(
                success=True,
                message=last_evaluation.message if last_evaluation else "Login executado",
                captcha_solution=captcha_solution,
    ...
    def _classify_attempt_feedback(
        ...
        # SECTION: Captcha keywords verificadas ANTES de credential keywords
        # para tratar mensagens ambíguas como "código da imagem incorreto"
    ...
    async def _navigate_to_filter_page(self) -> None:
        ...
        for selector in filter_config.menu_link_selectors:
            try:
                ...
                if await el.is_visible(timeout=3000):
                    ...
                    await self._human.reading_pause()
                    logger.info("Navegou para filtro via menu: %s", selector)
                    return
            except Exception:
                continue
        ...
        await self._human.reading_pause()
        logger.info("Navegou para filtro via URL: %s", filter_url)
    ...
    async def _fill_filter_fields(self, unit: str, date: str) -> None:
        ...
        if date:
            ...
            logger.info("Data preenchida: %s", date)
            await self._human.think()
    ...
    async def _select_dropdown_option(
        ...
        for method in [("label", value), ("value", value)]:
            try:
                ...
                logger.info("%s selecionado: %s", label, value)
                return
            except Exception:
                continue

        logger.warning("Falha ao selecionar %s: %s", label, value)
